...
export class ConsumerNotificationsController {
  ...
  registerDevice(@Body() dto: RegisterConsumerPushDeviceDto, @CurrentConsumerUser() userId: string) {
    if (!userId) throw new BadRequestException('Authenticated user is required');
    return this.push.registerConsumer(userId, dto.token, dto.platform, dto.deviceId);
  }

  @Post('devices/unregister')
  unregisterDevice(@Body() dto: UnregisterConsumerPushDeviceDto, @CurrentConsumerUser() userId: string) {
    if (!userId) throw new BadRequestException('Authenticated user is required');
