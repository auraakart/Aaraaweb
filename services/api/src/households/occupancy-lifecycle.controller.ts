...
class OperatorMoveInByPhoneDto{@IsUUID() unitId!:string;@IsString() @MaxLength(20) tenantPhone!:string;@IsEnum(UnitRelation) relation!:UnitRelation;@IsISO8601() effectiveAt!:string;@IsOptional() @IsString() @MaxLength(500) reason?:string;}
...
export class OccupancyLifecycleController{
  ...
  @Get('operator-context') @RequiresPermissions(AppPermission.OCCUPANCY_LIFECYCLE_READ) operatorContext(@CurrentTenant() societyId:string){return this.lifecycle.operatorContext(societyId);}
  ...
  @Get(':id/readiness') @RequiresPermissions(AppPermission.OCCUPANCY_LIFECYCLE_READ) readiness(@CurrentTenant() societyId:string,@Param('id',new ParseUUIDPipe()) id:string){return this.lifecycle.readiness(societyId,id);}
  ...
  @Post('move-ins/by-phone') @RequiresPermissions(AppPermission.OCCUPANCY_LIFECYCLE_MANAGE) moveInByPhone(@CurrentTenant() societyId:string,@CurrentUser() userId:string|undefined,@Body() dto:OperatorMoveInByPhoneDto){return this.lifecycle.requestMoveInByPhone(societyId,this.user(userId),{...dto,effectiveAt:this.date(dto.effectiveAt)});}
