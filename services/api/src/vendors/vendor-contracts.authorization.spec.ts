...
describe('VendorContractsController authorization',()=>{
 for(const method of ['list','history'] as const){it(`${method} requires SOCIETY_VENDORS_READ`,()=>expect(Reflect.getMetadata(PERMISSIONS_KEY,VendorContractsController.prototype[method])).toEqual([AppPermission.SOCIETY_VENDORS_READ]));}
