...
describe('VendorContractsService',()=>{
 ...
 it('fails closed when contract history is requested outside tenant',async()=>{const{prisma,service}=setup();prisma.$queryRaw.mockResolvedValueOnce([]);await expect(service.history('society-1','11111111-1111-1111-1111-111111111111')).rejects.toBeInstanceOf(NotFoundException);expect(prisma.$queryRaw).toHaveBeenCalledTimes(1)})
