import { Body, Controller, Post, UseGuards, createParamDecorator, ExecutionContext } from '@nestjs/common';
...
export class VisitorsController {
  ...
  @Post('verify') verify(@Body() dto: CredentialDto, @CurrentTenant() societyId: string) { return this.verification.verify(societyId, dto.credential); }
  @Post('check-in') checkIn(@Body() dto: CredentialDto, @CurrentTenant() societyId: string) { return this.verification.checkIn(societyId, dto.credential); }
  @Post('check-out') checkOut(@Body() dto: CredentialDto, @CurrentTenant() societyId: string) { return this.verification.checkOut(societyId, dto.credential); }
}
