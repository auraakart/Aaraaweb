...
export class ServicesMarketplaceService {
  ...
  private async assertEnabled(societyId: string) {
    if (!(await this.entitlements.isEnabled(societyId, ProductFeature.HOUSEHOLD_SERVICES))) {
      throw new ForbiddenException('Household services marketplace is not enabled for this society');
    }
  }
  ...
  listCategories() {
    return this.prisma.serviceCategory.findMany({ where: { active: true }, orderBy: [{ sortOrder: 'asc' }, { name: 'asc' }] });
  }
  ...
  createCategory(name: string, slug: string, sortOrder = 0) {
    return this.prisma.serviceCategory.create({
      data: { name: name.trim(), slug: slug.trim().toLowerCase(), sortOrder },
    });
  }
  ...
  verifyProvider(providerId: string) {
    return this.prisma.serviceProvider.update({
      where: { id: providerId },
      data: { verification: ProviderVerificationStatus.VERIFIED, active: true },
    });
  }
  ...
  async approveProviderForSociety(societyId: string, providerId: string, commissionBps = 1000) {
    ...
    return this.prisma.serviceProviderSociety.upsert({
      where: { societyId_providerId: { societyId, providerId } },
      create: { societyId, providerId, status: ProviderSocietyStatus.APPROVED, commissionBps, approvedAt: new Date() },
      update: { status: ProviderSocietyStatus.APPROVED, commissionBps, approvedAt: new Date() },
    });
  }
  ...
  async createOffering(providerId: string, categoryId: string, name: string, pricePaise: number, description?: string, durationMinutes?: number) {
    ...
    return this.prisma.serviceOffering.create({
      data: { providerId, categoryId, name: name.trim(), description: description?.trim() || null, pricePaise, durationMinutes },
    });
  }
  ...
  async book(societyId: string, residentUserId: string, unitId: string, offeringId: string, scheduledFrom: Date, scheduledUntil: Date, notes?: string) {
    ...
    return this.prisma.serviceBooking.create({
      ...
      include: { offering: true, provider: true },
    });
  }
  ...
  listMine(societyId: string, residentUserId: string) {
    return this.prisma.serviceBooking.findMany({
      ...
      include: { offering: { include: { category: true } }, provider: true, rating: true, accessRequest: true },
  ...
  async confirm(societyId: string, bookingId: string) {
    ...
    const request = await this.access.create(
      societyId,
      booking.residentUserId,
      booking.unitId,
      AccessSubjectType.SERVICE_PROVIDER,
      booking.provider.businessName,
      booking.provider.phone,
      booking.offering.name,
      { bookingId: booking.id, providerId: booking.providerId, offeringId: booking.offeringId },
    );
    ...
    const updated = await this.prisma.serviceBooking.update({
      ...
      include: { offering: true, provider: true, accessRequest: true },
    });
  ...
  async cancelMine(societyId: string, residentUserId: string, bookingId: string) {
    ...
    if (!booking) throw new NotFoundException('Service booking not found');
    if (![ServiceBookingStatus.REQUESTED, ServiceBookingStatus.CONFIRMED].includes(booking.status)) {
  ...
  async complete(societyId: string, bookingId: string) {
    ...
    if (!booking) throw new NotFoundException('Service booking not found');
    if (![ServiceBookingStatus.CONFIRMED, ServiceBookingStatus.IN_PROGRESS].includes(booking.status)) {
      throw new BadRequestException(`Booking is ${booking.status.toLowerCase()}`);
    }
  ...
  async rate(societyId: string, residentUserId: string, bookingId: string, score: number, comment?: string) {
    ...
    const booking = await this.prisma.serviceBooking.findFirst({
      where: { id: bookingId, societyId, residentUserId, status: ServiceBookingStatus.COMPLETED },
    });
    if (!booking) throw new NotFoundException('Completed booking not found');
    return this.prisma.serviceRating.create({
      data: { societyId, bookingId, providerId: booking.providerId, residentUserId, score, comment: comment?.trim() || null },
    });
  }
}
