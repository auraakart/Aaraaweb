...
describe('ConsumerProviderOperatorService', () => {
  ...
  it('scopes agent creation to the authenticated provider', async () => {
    ...
    await service.createMyAgent('33333333-3333-3333-3333-333333333333', { displayName: 'Ravi' });
  ...
  it('blocks dispatch transition for another providers assignment', async () => {
    const { prisma, dispatch, service } = setup();
    prisma.$queryRaw
      .mockResolvedValueOnce([verifiedProvider])
      .mockResolvedValueOnce([]);
