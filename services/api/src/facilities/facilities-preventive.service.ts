...
export class FacilitiesPreventiveService implements OnModuleInit,OnModuleDestroy{
  ...
  onModuleInit(){
    ...
    this.timer=setInterval(()=>void this.runAll().catch(e=>this.logger.error(`Preventive maintenance generation failed: ${safeOperationalError(e)}`)),interval);
    this.timer.unref();
    void this.runAll().catch(e=>this.logger.error(`Initial preventive maintenance generation failed: ${safeOperationalError(e)}`));
  }
