...
export class AccountingService {
    ...
    try {
      const rows = await this.prisma.$queryRaw<Array<Record<string, unknown>>>(Prisma.sql`
        ...
        VALUES (
          ...
          ${code},
          ${name},
          ${input.type}::"LedgerAccountType",
          ${input.parentAccountId ?? null}::uuid,
  ...
  async createPeriod(societyId: string, input: PeriodInput) {
    ...
    const overlap = await this.prisma.$queryRaw<Array<{ id: string }>>(Prisma.sql`
      ...
      WHERE "societyId" = ${societyId}::uuid
        AND daterange("startsOn", "endsOn", '[]') && daterange(${input.startsOn}::date, ${input.endsOn}::date, '[]')
      LIMIT 1
    `);
  ...
  listJournals(societyId: string) {
    return this.prisma.$queryRaw(Prisma.sql`
      SELECT je."id", je."entryNumber", je."entryDate", je."description", je."status",
             je."sourceType", je."sourceId", je."externalReference", je."postedAt",
  ...
  async createDraft(societyId: string, userId: string, input: JournalInput) {
    ...
    return this.prisma.$transaction(async (tx) => {
      const periods = await tx.$queryRaw<Array<{ id: string; startsOn: Date; endsOn: Date }>>(Prisma.sql`
        ...
        WHERE "id" = ${input.periodId}::uuid AND "societyId" = ${societyId}::uuid
        LIMIT 1
      `);
      ...
      const headers = await tx.$queryRaw<Array<{ id: string }>>(Prisma.sql`
        INSERT INTO "JournalEntry" (
          "societyId", "periodId", "entryNumber", "entryDate", "description",
          "sourceType", "sourceId", "externalReference", "createdByUserId"
        ) VALUES (
          ${societyId}::uuid, ${input.periodId}::uuid, ${input.entryNumber.trim().toUpperCase()}, ${input.entryDate}::date,
          ${input.description.trim()}, ${input.sourceType?.trim() || null}, ${input.sourceId?.trim() || null},
          ${input.externalReference?.trim() || null}, ${userId}::uuid
        ) RETURNING "id"
      `);
      ...
      for (const line of input.lines) {
        await tx.$executeRaw(Prisma.sql`
          INSERT INTO "JournalLine" (
            ...
            ${line.unitId ?? null}::uuid, ${line.description?.trim() || null}, ${line.debitPaise}, ${line.creditPaise}
          )
        `);
      }
  ...
  async postJournal(societyId: string, userId: string, journalId: string) {
    return this.prisma.$transaction(async (tx) => {
      const rows = await tx.$queryRaw<JournalRow[]>(Prisma.sql`
        SELECT je.* FROM "JournalEntry" je
        WHERE je."id" = ${journalId}::uuid AND je."societyId" = ${societyId}::uuid
        FOR UPDATE
      `);
      ...
      const periods = await tx.$queryRaw<Array<{ status: string; startsOn: Date; endsOn: Date }>>(Prisma.sql`
        SELECT "status", "startsOn", "endsOn" FROM "AccountingPeriod"
        WHERE "id" = ${journal.periodId}::uuid AND "societyId" = ${societyId}::uuid
        FOR UPDATE
      `);
  ...
  async reverseJournal(societyId: string, userId: string, journalId: string, input: ReverseInput) {
    return this.prisma.$transaction(async (tx) => {
      const sourceRows = await tx.$queryRaw<JournalRow[]>(Prisma.sql`
      ...
      const periodRows = await tx.$queryRaw<Array<{ id: string; status: string; startsOn: Date; endsOn: Date }>>(Prisma.sql`
      ...
      const reversalRows = await tx.$queryRaw<Array<{ id: string }>>(Prisma.sql`
        INSERT INTO "JournalEntry" (
          ...
          "sourceType", "sourceId", "createdByUserId", "postedByUserId", "postedAt", "reversalOfEntryId"
        ) VALUES (
          ${societyId}::uuid, ${periodRows[0].id}::uuid, ${input.entryNumber.trim().toUpperCase()}, ${input.entryDate}::date,
          ${`Reversal: ${input.reason.trim()}`}, 'POSTED', 'JOURNAL_REVERSAL', ${journalId}, ${userId}::uuid, ${userId}::uuid,
          CURRENT_TIMESTAMP, ${journalId}::uuid
        ) RETURNING "id"
      `);
      ...
      await tx.$executeRaw(Prisma.sql`
        ...
        FROM "JournalLine"
        WHERE "entryId" = ${journalId}::uuid AND "societyId" = ${societyId}::uuid
      `);

      await tx.$executeRaw(Prisma.sql`
        UPDATE "JournalEntry"
        SET "status" = 'REVERSED', "reversedAt" = CURRENT_TIMESTAMP, "updatedAt" = CURRENT_TIMESTAMP
        WHERE "id" = ${journalId}::uuid AND "societyId" = ${societyId}::uuid
      `);
