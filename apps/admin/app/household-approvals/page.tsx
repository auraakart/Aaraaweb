...
export default function HouseholdApprovalsPage(){
  ...
  const load=useCallback(async(s:Session)=>{setLoading(true);setError('');try{setRows(await api<RequestRow[]>('/household-change-requests/admin/pending',{},s))}catch(e){setError(e instanceof Error?e.message:'Could not load household approval requests')}finally{setLoading(false)}},[])
  useEffect(()=>{const s=getSession();setSession(s);if(s&&allowed(s))void load(s);else setLoading(false)},[load])
  const review=async(row:RequestRow,action:'approve'|'reject')=>{if(!session)return;setBusy(row.id);setError('');try{await api(`/household-change-requests/admin/${row.id}/${action}`,{method:'POST',body:'{}'},session);await load(session)}catch(e){setError(e instanceof Error?e.message:'Could not review request')}finally{setBusy('')}}
