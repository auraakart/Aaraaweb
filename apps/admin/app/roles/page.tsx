...
export default function RolesPage(){
  ...
  const deactivate=async(m:Membership)=>{if(!session||!await operatorConfirm(`Deactivate ${m.role.replaceAll('_',' ')} for ${m.user.name||m.user.phone}? Active sessions for this society will be revoked.`))return;setBusy(true);setError('');void api(session,`/society-roles/${m.id}/deactivate`,{method:'PATCH',body:'{}'}).then(()=>load(session)).catch(e=>setError(e instanceof Error?e.message:'Could not deactivate role')).finally(()=>setBusy(false))}
