...
const assignableRoles=['COMMITTEE_MEMBER','FACILITY_MANAGER','ACCOUNTANT','AUDITOR','SECURITY_SUPERVISOR','SECURITY_GUARD','STAFF']
...
export default function RolesPage(){
  ...
  const load=useCallback(async(s:Session)=>{setLoading(true);setError('');try{setRows(await api<Membership[]>('/society-roles',{},s))}catch(e){setError(e instanceof Error?e.message:'Could not load society roles')}finally{setLoading(false)}},[])
  useEffect(()=>{const s=getSession();setSession(s);if(s&&allowed(s))void load(s);else setLoading(false)},[load])
  const provision=(e:FormEvent)=>{e.preventDefault();if(!session)return;setBusy(true);setError('');void api('/society-roles',{method:'POST',body:JSON.stringify({name:name.trim(),phone:phone.trim(),role})},session).then(()=>{setName('');setPhone('+91');return load(session)}).catch(e=>setError(e instanceof Error?e.message:'Could not assign role')).finally(()=>setBusy(false))}
  const deactivate=async(m:Membership)=>{if(!session||!await operatorConfirm(`Deactivate ${m.role.replaceAll('_',' ')} for ${m.user.name||m.user.phone}? Active sessions for this society will be revoked.`))return;setBusy(true);setError('');void api(`/society-roles/${m.id}/deactivate`,{method:'PATCH',body:'{}'},session).then(()=>load(session)).catch(e=>setError(e instanceof Error?e.message:'Could not deactivate role')).finally(()=>setBusy(false))}
