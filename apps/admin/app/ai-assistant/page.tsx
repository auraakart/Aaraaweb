...
export default function AiAssistantPage(){
  ...
  const[topic,setTopic]=useState(''),[language,setLanguage]=useState<'en-IN'|'hi-IN'|'ta-IN'>('en-IN'),[draft,setDraft]=useState<NoticeDraft|null>(null),[audit,setAudit]=useState<AuditItem[]>([]),[centre,setCentre]=useState<ActionCentre|null>(null)
  useEffect(()=>{const current=session();setSession(current);if(current){void loadCentre(current);if(['SUPER_ADMIN','SOCIETY_ADMIN','COMMITTEE_MEMBER','FACILITY_MANAGER','ACCOUNTANT','AUDITOR','SECURITY_SUPERVISOR'].includes(current.role)){void loadAudit(current)}}},[])
