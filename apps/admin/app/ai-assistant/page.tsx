...
type ActionCentre={cards:ActionCard[];grounded:boolean;mutationPerformed:boolean}
...
export default function AiAssistantPage(){
  ...
  return <main style={{maxWidth:1120,margin:'0 auto',padding:'28px 22px 80px'}}>
    ...
    {centre&&centre.cards.length>0&&<section style={panel}><h2 style={{marginTop:0}}>Action Centre</h2><p>Permission-scoped signals from authoritative society records. Selecting a card only prepares a grounded query; it does not execute an action.</p><div style={cardGrid}>{centre.cards.map(card=><button key={card.id} type="button" style={actionCard} onClick={()=>setMessage(card.prompt)}><span style={severity(card.severity)}>{card.severity}</span><strong>{card.title}</strong><span>{card.summary}</span><small>{card.domain} · {card.sources.join(', ')}</small></button>)}</div></section>}
