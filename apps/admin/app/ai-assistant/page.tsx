...
type ActionCard={id:string;domain:string;severity:'LOW'|'MEDIUM'|'HIGH';title:string;summary:string;prompt:string;sources:string[];metrics:Record<string,number|string|null>}
type ActionCentre={cards:ActionCard[];generatedAt?:string;grounded:boolean;mutationPerformed:boolean}
...
export default function AiAssistantPage(){
  ...
  return <main style={{maxWidth:1120,margin:'0 auto',padding:'28px 22px 80px'}}>
    ...
    {centre&&centre.cards.length>0&&<section style={panel}><h2 style={{marginTop:0}}>Daily operations briefing</h2><p>Permission-scoped priorities from authoritative society records. Selecting a card only prepares a grounded query; it does not execute an action.</p>{centre.generatedAt&&<p><small>Updated {new Date(centre.generatedAt).toLocaleString('en-IN')} · read only</small></p>}<div style={cardGrid}>{centre.cards.map(card=><button key={card.id} type="button" style={actionCard} onClick={()=>setMessage(card.prompt)}><span style={severity(card.severity)}>{card.severity}</span><strong>{card.title}</strong><span>{card.summary}</span><small>{card.domain} · {card.sources.join(', ')}</small></button>)}</div></section>}
