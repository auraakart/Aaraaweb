...
export default function ProviderMediaPage(){
  ...
  const remove=async(item:ProviderMedia)=>{if(!session)return;if(!await operatorConfirm('Remove this image from your provider profile?'))return;setBusy(true);setError('');setNotice('');try{await api(session,`/provider/services/media/${item.id}`,{method:'DELETE'});setNotice('Image removed from provider media.');await load(session)}catch(e){setError(e instanceof Error?e.message:'Image could not be removed')}finally{setBusy(false)}}
