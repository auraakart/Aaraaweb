...
export default function WaiverWorkspace(){
 ...
 const load=useCallback(async(s:Session)=>{setError('');try{const[w,r]=await Promise.all([api<Waiver[]>('/accounting/waivers/requests',{},s),api<Receivable[]>('/accounting/receivables',{},s)]);setRows(w);setReceivables(r)}catch(e){setError(e instanceof Error?e.message:'Could not load waiver controls')}},[])
 useEffect(()=>{const s=getSession();setSession(s);if(s)void load(s)},[load])
 const canManage=!!session&&manageRoles.has(session.role)

 async function submit(e:FormEvent){e.preventDefault();if(!session||!canManage)return;const rupees=Number(amount);if(!Number.isFinite(rupees)||rupees<=0){setError('Enter a positive waiver amount.');return}setBusy(true);setError('');setSuccess('');try{await api('/accounting/waivers/requests',{method:'POST',body:JSON.stringify({receivableId,amountPaise:Math.round(rupees*100),reason,entryDate:today(),journalEntryNumber:entryNumber,requestKey})},session);setAmount('');setReason('');setEntryNumber('');setRequestKey('');setSuccess('Waiver request submitted for maker–checker review.');await load(session)}catch(e){setError(e instanceof Error?e.message:'Could not submit waiver request')}finally{setBusy(false)}}
 ...
 async function review(e:FormEvent){e.preventDefault();if(!session||!canManage||!reviewId)return;const reason=reviewReason.trim();if(reviewDecision==='reject'&&!reason){setError('Enter a rejection reason.');return}setBusy(true);setError('');setSuccess('');try{await api(`/accounting/waivers/requests/${reviewId}/${reviewDecision}`,{method:'POST',body:reviewDecision==='reject'?JSON.stringify({reason}):undefined},session);setReviewId('');setReviewReason('');setSuccess(`Waiver request ${reviewDecision==='approve'?'approved':'rejected'}.`);await load(session)}catch(e){setError(e instanceof Error?e.message:'Could not review waiver request')}finally{setBusy(false)}}
