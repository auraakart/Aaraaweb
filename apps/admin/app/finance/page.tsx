...
type LatePreview={receivableId:string;receivableNumber:string;baseOutstandingPaise:string;feePaise:string;lateFeeMode:string;dueDate:string}[]
type LateBatch={id:string;asOfDate:string;status:string;assessmentCount?:number;totalFeePaise?:string;createdAt?:string}
type Unapplied={paymentCount:number;unappliedPaise:string}
type PaymentAvailability={paymentId:string;status:string;amountPaise:string;allocatedPaise:string;grossAllocatedPaise:string;reversedPaise:string;refundedPaise:string;unallocatedPaise:string}
...
export default function FinanceWorkspace(){
  ...
  const[latePreview,setLatePreview]=useState<LatePreview>([]),[batches,setBatches]=useState<LateBatch[]>([]),[unapplied,setUnapplied]=useState<Unapplied|null>(null),[executionReadiness,setExecutionReadiness]=useState<OperationalReadiness|null>(null),[treasurer,setTreasurer]=useState<TreasurerControl|null>(null)
  const[asOf,setAsOf]=useState(today()),[paymentId,setPaymentId]=useState(''),[payment,setPayment]=useState<PaymentAvailability|null>(null),[allocations,setAllocations]=useState<Allocation[]>([]),[allocationReversals,setAllocationReversals]=useState<AllocationReversal[]>([]),[paymentRefunds,setPaymentRefunds]=useState<PaymentRefund[]>([])
  ...
  const load=useCallback(async(s:AdminSession)=>{setLoading(true);setError('');try{const[r,a,cr,j,p,lb,u,er,tc]=await Promise.all([
    adminApi<Receivable[]>(s,'/accounting/receivables'),adminApi<Ageing>(s,`/accounting/receivables/ageing?asOf=${today()}`),adminApi<Rule[]>(s,'/accounting/receivables/charge-rules'),adminApi<Journal[]>(s,'/accounting/journals'),adminApi<Period[]>(s,'/accounting/periods'),adminApi<LateBatch[]>(s,'/accounting/late-fees/batches'),adminApi<Unapplied>(s,'/accounting/late-fees/unapplied-cash'),adminApi<OperationalReadiness>(s,'/accounting/finance-operations/operational-readiness'),adminApi<TreasurerControl>(s,'/accounting/finance-operations/treasurer-control-centre')]);setReceivables(r);setAgeing(a);setRules(cr);setJournals(j);setPeriods(p);setSelectedPeriodId(current=>current&&p.some(period=>period.id===current)?current:(p.find(period=>period.status==='OPEN')??p[0])?.id??'');setBatches(lb);setUnapplied(u);setExecutionReadiness(er);setTreasurer(tc)}catch(e){setError(e instanceof Error?e.message:'Could not load finance workspace')}finally{setLoading(false)}},[])
  ...
  const unappliedCash=Number(unapplied?.unappliedPaise??0)
  ...
  async function inspectPayment(e:FormEvent){e.preventDefault();if(!session||!paymentId.trim())return;setBusy(true);setError('');try{const id=paymentId.trim();const[p,a,r,f]=await Promise.all([adminApi<PaymentAvailability>(session,`/accounting/settlements/payments/${id}/availability`),adminApi<Allocation[]>(session,`/accounting/settlements/payments/${id}/allocations`),adminApi<AllocationReversal[]>(session,`/accounting/payment-exceptions/payments/${id}/allocation-reversals`),adminApi<PaymentRefund[]>(session,`/accounting/payment-exceptions/payments/${id}/refunds`)]);setPayment(p);setAllocations(a);setAllocationReversals(r);setPaymentRefunds(f)}catch(e){setPayment(null);setAllocations([]);setAllocationReversals([]);setPaymentRefunds([]);setError(e instanceof Error?e.message:'Could not inspect payment')}finally{setBusy(false)}}
  ...
  return <PageShell>
    ...
    <section style={panel}><h2>Receivables summary</h2><EvidenceGrid items={[
      ...
      {id:'unapplied-cash',label:'Unapplied cash',value:money(unapplied?.unappliedPaise)},
    ]}/></section>
  ...
  {id:'current',label:'Current',value:money(ageing.currentPaise)},{id:'1-30',label:'1–30 days',value:money(ageing.days1To30Paise)},{id:'31-60',label:'31–60 days',value:money(ageing.days31To60Paise)},{id:'61-90',label:'61–90 days',value:money(ageing.days61To90Paise)},{id:'90-plus',label:'90+ days',value:money(ageing.days90PlusPaise)}
    ...
    <section style={panel}><div style={sectionHeader}><div><h2 style={{margin:'0 0 4px'}}>Late-fee control</h2><small>Preview first; posting is explicit and auditable.</small></div><ActionBar label="Late-fee actions"><FormField label="As of" type="date" value={asOf} onChange={e=>{setAsOf(e.target.value);setLateFeeConfirmed(false)}}/><SecondaryButton disabled={busy} onClick={()=>void previewLateFees()}>Preview</SecondaryButton>{canManage&&<DangerButton disabled={busy||latePreview.length===0||!lateFeeConfirmed} onClick={()=>void applyLateFees()}>Apply batch</DangerButton>}</ActionBar></div>{latePreview.length>0&&<div style={{marginTop:14}}>{canManage&&<label style={confirmRow}><input type="checkbox" checked={lateFeeConfirmed} onChange={e=>setLateFeeConfirmed(e.target.checked)}/><span>I confirm this late-fee batch will post accounting journals for the previewed receivables.</span></label>}<b>{latePreview.length} eligible receivable{latePreview.length===1?'':'s'}</b><div style={{display:'grid',gap:8,marginTop:8}}>{latePreview.slice(0,30).map(x=><div key={x.receivableId} style={row}><span>{x.receivableNumber} · due {new Date(x.dueDate).toLocaleDateString('en-IN')}</span><span>{money(x.baseOutstandingPaise)} → <b>{money(x.feePaise)} fee</b></span></div>)}</div></div>}<div style={{marginTop:18}}><small>Recent batches</small>{batches.length===0?<p>No late-fee batches yet.</p>:batches.slice(0,10).map(b=><div key={b.id} style={row}><span>{new Date(b.asOfDate).toLocaleDateString('en-IN')} · {b.status}</span><span>{b.assessmentCount??0} assessments · {money(b.totalFeePaise)}</span></div>)}</div></section>

    <section id="payment-allocation" style={panel}><h2 style={{marginTop:0}}>Payment allocation</h2><p>Inspect captured cash, net allocation, reversals and refunds before allocating any remaining amount.</p><form onSubmit={inspectPayment} style={inlineForm}><FormField label="Payment ID" value={paymentId} onChange={e=>setPaymentId(e.target.value)} placeholder="UUID"/><SecondaryButton type="submit" disabled={busy||!paymentId.trim()}>Inspect payment</SecondaryButton></form>{payment&&<div style={{marginTop:16}}><div style={metricGrid}><Metric label="Payment amount" value={money(payment.amountPaise)}/><Metric label="Gross allocated" value={money(payment.grossAllocatedPaise)}/><Metric label="Reversed" value={money(payment.reversedPaise)}/><Metric label="Net allocated" value={money(payment.allocatedPaise)}/><Metric label="Refunded" value={money(payment.refundedPaise)}/><Metric label="Available" value={money(payment.unallocatedPaise)}/></div>{canManage&&<form onSubmit={allocate} style={allocationForm}><SelectField label="Receivable" value={allocationReceivable} onChange={e=>setAllocationReceivable(e.target.value)}><option value="">Choose open receivable</option>{receivables.filter(r=>Number(r.outstandingPaise)>0).map(r=><option key={r.id} value={r.id}>{r.receivableNumber} · {money(r.outstandingPaise)}</option>)}</SelectField><FormField label="Amount (₹)" type="number" min="0.01" step="0.01" value={allocationAmount} onChange={e=>setAllocationAmount(e.target.value)}/><PrimaryButton type="submit" disabled={busy||!allocationReceivable||!allocationAmount}>Allocate</PrimaryButton></form>}<div style={{marginTop:14}}><small>Allocation history</small>{allocations.length===0?<p>No allocations yet.</p>:allocations.map(a=><div key={a.id} style={row}><span>{a.receivableId}<br/><small>{Number(a.reversedPaise)>0?money(a.reversedPaise)+' reversed · '+money(a.reversiblePaise)+' net active':'No reversal recorded'}</small></span><span>{money(a.amountPaise)} gross · {new Date(a.allocatedAt).toLocaleString('en-IN')}</span></div>)}</div><div style={{marginTop:14}}><small>Allocation reversals</small>{allocationReversals.length===0?<p>No allocation reversals.</p>:allocationReversals.map(r=><div key={r.id} style={row}><span><b>{money(r.amountPaise)} reversed</b> · {r.receivableId}<br/><small>{r.reason}</small></span><span>{new Date(r.reversedAt).toLocaleString('en-IN')}</span></div>)}</div><div style={{marginTop:14}}><small>Refund history</small>{paymentRefunds.length===0?<p>No refunds recorded.</p>:paymentRefunds.map(r=><div key={r.id} style={row}><span><b>{money(r.amountPaise)} refunded</b><br/><small>{r.reason}{r.providerReference?' · '+r.providerReference:''}</small></span><span>{new Date(r.refundedAt).toLocaleString('en-IN')}</span></div>)}</div></div>}</section>
