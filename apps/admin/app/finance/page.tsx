...
export default function FinanceWorkspace(){
  ...
  {id:'current',label:'Current',value:money(ageing.currentPaise)},{id:'1-30',label:'1–30 days',value:money(ageing.days1To30Paise)},{id:'31-60',label:'31–60 days',value:money(ageing.days31To60Paise)},{id:'61-90',label:'61–90 days',value:money(ageing.days61To90Paise)},{id:'90-plus',label:'90+ days',value:money(ageing.days90PlusPaise)}
    ...
    <section id="adjustment-notes" style={panel}><div style={sectionHeader}><div><h2 style={{margin:'0 0 4px'}}>Credit & debit notes</h2><small>Append-only resident-visible adjustment evidence. Waivers remain maker–checker controlled separately.</small></div>{noteReceivable&&<StatusPill label={notes.length?`${notes.length} NOTE${notes.length===1?'':'S'}`:'NO NOTES'} tone={notes.length?'neutral':'success'}/>}</div><div style={formGrid}><SelectField label="Receivable" value={noteReceivable} onChange={e=>{setNoteReceivable(e.target.value);void loadNotes(e.target.value)}}><option value="">Choose receivable</option>{receivables.slice(0,100).map(r=><option key={r.id} value={r.id}>{r.receivableNumber} · {money(r.outstandingPaise)} outstanding</option>)}</SelectField>{canManage&&<><SelectField label="Note type" value={noteType} onChange={e=>setNoteType(e.target.value as 'DEBIT'|'CREDIT')}><option value="CREDIT">Credit note</option><option value="DEBIT">Debit note</option></SelectField><FormField label="Note number" value={noteNumber} onChange={e=>setNoteNumber(e.target.value)} placeholder="CN-2026-0001"/><FormField label="Entry date" type="date" value={noteDate} onChange={e=>setNoteDate(e.target.value)}/><FormField label="Amount (₹)" type="number" min="0.01" step="0.01" value={noteAmount} onChange={e=>setNoteAmount(e.target.value)}/><FormField label="Reason" value={noteReason} onChange={e=>setNoteReason(e.target.value)}/><PrimaryButton disabled={busy||!noteReceivable||!noteNumber.trim()||!noteAmount||!noteReason.trim()} onClick={e=>void createNote(e as unknown as FormEvent)}>Post note</PrimaryButton></>}</div>{noteReceivable&&<div style={{marginTop:16}}>{notes.length===0?<EmptyState title="No adjustment notes" description="No credit, debit or waiver evidence has been posted against this receivable."/>:notes.map(n=><div key={n.id} style={row}><span><b>{n.noteNumber}</b> · {n.documentType.replaceAll('_',' ')}<br/><small>{n.reason} · {new Date(n.entryDate).toLocaleDateString('en-IN')} · {n.journalStatus}{n.residentVisible?' · resident visible':''}</small></span><b>{money(n.amountPaise)}</b></div>)}</div>}</section>
