...
export default function ReconciliationPage(){
 ...
 async function selectCase(c:Case){
  ...
  try{const next=await api<Operation[]>(s,`/accounting/payment-reconciliation/payments/${c.paymentId}/operations`);if(requestId===operationRequest.current)setOps(next)}
 ...
 async function openCase(e:FormEvent){e.preventDefault();if(!s||!canManage||!paymentId.trim()||!provider.trim())return;setBusy(true);setError('');setSuccess('');try{const c=await api<Case>(s,`/accounting/payment-reconciliation/payments/${paymentId.trim()}/cases`,{method:'POST',body:JSON.stringify({provider:provider.trim()})});await load(s);await selectCase(c);setPaymentId('');setSuccess('Reconciliation case opened/refreshed.')}catch(e){setError(e instanceof Error?e.message:'Could not open reconciliation case')}finally{setBusy(false)}}
 async function observe(){if(!s||!canManage||!selected||!status.trim())return;const p=Number(amount);if(!Number.isFinite(p)||p<0)return setError('Enter a valid observed amount.');setBusy(true);setError('');setSuccess('');try{const c=await api<Case>(s,`/accounting/payment-reconciliation/cases/${selected.id}/observe`,{method:'POST',body:JSON.stringify({providerPaymentId:providerPaymentId.trim()||undefined,observedProviderStatus:status.trim(),observedAmountPaise:Math.round(p*100)})});setSelected(c);setSuccess('Provider observation recorded.');await load(s)}catch(e){setError(e instanceof Error?e.message:'Could not record provider observation')}finally{setBusy(false)}}
 async function resolve(){if(!s||!canManage||!selected)return;const reason=resolutionReason.trim();if(!reason)return setError('Enter a resolution reason before resolving the case.');setBusy(true);setError('');setSuccess('');try{const c=await api<Case>(s,`/accounting/payment-reconciliation/cases/${selected.id}/resolve`,{method:'POST',body:JSON.stringify({reason})});setSelected(c);setResolutionReason('');setSuccess('Reconciliation case resolved.');await load(s)}catch(e){setError(e instanceof Error?e.message:'Could not resolve case')}finally{setBusy(false)}}
 async function requestOp(type:'STATUS_QUERY'|'REFUND'){if(!s||!canManage||!selected)return;let amountPaise:number|undefined;if(type==='REFUND'){const r=Number(refundAmount);if(!Number.isFinite(r)||r<=0)return setError('Enter a positive refund amount.');amountPaise=Math.round(r*100)}setBusy(true);setError('');setSuccess('');try{await api(s,`/accounting/payment-reconciliation/payments/${selected.paymentId}/operations`,{method:'POST',body:JSON.stringify({operationType:type,provider:selected.provider,amountPaise,idempotencyKey:`admin-${type.toLowerCase()}-${selected.paymentId}-${Date.now()}`})});if(type==='REFUND')setRefundAmount('');setOps(await api<Operation[]>(s,`/accounting/payment-reconciliation/payments/${selected.paymentId}/operations`));setSuccess(type==='REFUND'?'Refund operation requested.':'Status query requested.')}catch(e){setError(e instanceof Error?e.message:'Could not create gateway operation')}finally{setBusy(false)}}
