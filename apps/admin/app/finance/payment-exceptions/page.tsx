...
export default function PaymentExceptions(){
 ...
 async function inspect(e?:FormEvent){e?.preventDefault();if(!s||!paymentId.trim())return;setBusy(true);setError('');try{const id=paymentId.trim();const[x,a,r,f]=await Promise.all([api<Snapshot>(`/accounting/payment-exceptions/payments/${id}`,{},s),api<Allocation[]>(`/accounting/settlements/payments/${id}/allocations`,{},s),api<Reversal[]>(`/accounting/payment-exceptions/payments/${id}/allocation-reversals`,{},s),api<Refund[]>(`/accounting/payment-exceptions/payments/${id}/refunds`,{},s)]);setSnap(x);setAllocs(a);setRevs(r);setRefunds(f)}catch(e){setSnap(null);setAllocs([]);setRevs([]);setRefunds([]);setError(e instanceof Error?e.message:'Could not inspect payment')}finally{setBusy(false)}}
