...
export default function ExternalServicesOperationsPage(){
  const[session,setSession]=useState<Session|null>(null),[summary,setSummary]=useState<Summary|null>(null),[attention,setAttention]=useState<AttentionQueue|null>(null),[loading,setLoading]=useState(true),[error,setError]=useState('')
  const load=useCallback(async(s:Session)=>{setLoading(true);setError('');try{const[summaryData,attentionData]=await Promise.all([getJson<Summary>(s,'/platform/services/operations/summary'),getJson<AttentionQueue>(s,'/platform/services/operations/attention')]);setSummary(summaryData);setAttention(attentionData)}catch(e){setError(e instanceof Error?e.message:'Operations data could not be loaded')}finally{setLoading(false)}},[])
