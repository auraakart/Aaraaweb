...
export default function VisitorParkingPermitsPage(){
  ...
  async function closePermit(id:string,action:'cancel'|'complete'){if(!session||!canManage)return;setBusy(true);setError('');try{await api(`/parking/v2/permits/${id}/${action}`,{method:'PATCH',body:JSON.stringify({})},session);await load()}catch(err){setError(err instanceof Error?err.message:`Could not ${action} parking permit`)}finally{setBusy(false)}}
