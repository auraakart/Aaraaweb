...
export default function ParcelsAdminPage(){
 ...
 async function load(){if(!s||!canUse)return;setBusy(true);setError('');try{const[p,r]=await Promise.all([api<Parcel[]>('/parcels/desk',{},s),api<Recipient[]>('/parcels/desk/recipients',{},s)]);setParcels(p);setRecipients(r);if(!recipientKey&&r[0])setRecipientKey(`${r[0].unitId}|${r[0].userId}`)}catch(e){setError(e instanceof Error?e.message:'Could not load parcel desk')}finally{setBusy(false)}}
 useEffect(()=>{void load()},[])
 async function intake(){if(!s||!recipientKey)return;const[unitId,recipientUserId]=recipientKey.split('|');setBusy(true);setError('');setOk('');try{await api('/parcels/desk',{method:'POST',body:JSON.stringify({unitId,recipientUserId,courierName:courier.trim()||undefined,trackingReference:tracking.trim()||undefined,notes:notes.trim()||undefined})},s);setCourier('');setTracking('');setNotes('');setOk('Parcel received and resident notified.');await load()}catch(e){setError(e instanceof Error?e.message:'Could not receive parcel')}finally{setBusy(false)}}
 async function remind(id:string){if(!s)return;setBusy(true);setError('');try{await api(`/parcels/desk/${id}/remind`,{method:'POST'},s);setOk('Reminder sent.');await load()}catch(e){setError(e instanceof Error?e.message:'Could not send reminder')}finally{setBusy(false)}}
 async function returnParcel(id:string){if(!s)return;const reason=await operatorPrompt('Return reason');if(!reason?.trim())return;setBusy(true);setError('');try{await api(`/parcels/desk/${id}/return`,{method:'PATCH',body:JSON.stringify({reason:reason.trim()})},s);setOk('Parcel marked returned to sender.');await load()}catch(e){setError(e instanceof Error?e.message:'Could not return parcel')}finally{setBusy(false)}}
