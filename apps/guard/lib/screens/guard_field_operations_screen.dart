...
class _GuardFieldOperationsScreenState extends State<GuardFieldOperationsScreen>{
  ...
  List<Map<String,dynamic>> overstays=const [],watchlist=const [],passes=const [],checkpoints=const [],incidents=const [];

  @override void initState(){super.initState();void load();}
  Future<void> load() async {setState(()=>loading=true);try{final values=await Future.wait([client.summary(),client.overstays(),client.watchlist(),client.passes(),client.checkpoints(),client.incidents()]);if(!mounted)return;setState((){summary=values[0] as Map<String,dynamic>;overstays=values[1] as List<Map<String,dynamic>>;watchlist=values[2] as List<Map<String,dynamic>>;passes=values[3] as List<Map<String,dynamic>>;checkpoints=values[4] as List<Map<String,dynamic>>;incidents=values[5] as List<Map<String,dynamic>>;error=null;});}catch(e){if(mounted)setState(()=>error=e.toString());}finally{if(mounted)setState(()=>loading=false);}}
  ...
  @override Widget build(BuildContext context){
    ...
    return Scaffold(appBar:AppBar(title:const Text('Field operations'),actions:[IconButton(onPressed:busy?null:load,icon:const Icon(Icons.refresh_rounded))]),body:SafeArea(child:loading?const Center(child:CircularProgressIndicator()):RefreshIndicator(onRefresh:load,child:ListView(padding:const EdgeInsets.fromLTRB(16,10,16,40),children:[
      ...
      const SizedBox(height:18),_sectionHeader('Incidents',Icons.report_problem_outlined,TextButton.icon(onPressed:busy?_none:_createIncident,icon:const Icon(Icons.add_alert_rounded),label:const Text('REPORT'))),...incidents.take(30).map((x)=>_tile('${x['severity']} · ${x['title']}', '${x['category']} · ${x['status']}',Icons.warning_amber_rounded)),if(incidents.isEmpty)_empty('No incidents recorded.'),
    ]))));
  }
  ...
  Future<void> _createIncident()async{final result=await showDialog<Map<String,String>>(context:context,builder:(context)=>const _IncidentDialog());if(result==null)return;await run(()=>client.createIncident(severity:result['severity']!,category:result['category']!,title:result['title']!,description:result['description'],gateId:widget.controller.gateId).then((_){}));}
