...
class QueuedGateAction {
  const QueuedGateAction({required this.type, required this.gateId, required this.credential, required this.idempotencyKey, required this.createdAt});
  final String type;
  final String gateId;
...
class OfflineActionQueue {
  ...
  Future<List<QueuedGateAction>> read() async {
    ...
    return decoded.whereType<Map>().map((item) => QueuedGateAction.fromJson(Map<String, dynamic>.from(item))).where((item) => item.idempotencyKey.isNotEmpty).toList();
  }
