...
class _ResidentSessionGateState extends State<_ResidentSessionGate> {
  ...
  void _ensureDataController() {
    ...
    final ResidentRepository repository = widget.authController.isDemoSession ? DemoResidentRepository() : ResidentRepository(ApiClient(baseUrl: widget.apiBaseUrl, accessToken: session.accessToken));
    _dataController = ResidentDataController(repository, activeUnitId: session.activeUnitId);
  }

  Future<void> _signOut() async { await _dataController?.stopPushNotifications(); await widget.authController.signOut(); }
  Future<void> _switchProperty(SocietyMembershipOption membership, PropertySummary? property) async { await _dataController?.stopPushNotifications(); await widget.authController.switchProperty(membership, property); }

  @override
  void dispose() { _dataController?.dispose(); super.dispose(); }
    ...
    return AnimatedBuilder(
      ...
      builder: (context, _) {
        ...
        final pages = <Widget>[
          ...
          ProfileScreen(controller: controller, onSignOut: widget.onSignOut, canManageFamilyMembers: widget.canManageFamilyMembers, propertyContexts: widget.propertyContexts, currentSocietyId: widget.currentSocietyId, currentUnitId: widget.currentUnitId, onSwitchProperty: widget.onSwitchProperty),
        ];
        ...
        return Scaffold(
          ...
          bottomNavigationBar: NavigationBar(selectedIndex: _index, onDestinationSelected: _open, destinations: const [
