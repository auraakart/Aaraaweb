        ...
        child: ListView(
          ...
          children: [
            ...
            FilledButton.tonalIcon(
              onPressed: () => Navigator.of(context).push(MaterialPageRoute(builder: (_) => CommunityPollsScreen(repository: controller.repository))),
              icon: const Icon(Icons.poll_outlined),
              label: const Text('Community polls'),
            ),
            ...
            if (controller.loading && controller.notices.isEmpty)
              ...
              for (final notice in controller.notices) ...[
                _NoticeCard(notice: notice, onTap: () => Navigator.of(context).push(MaterialPageRoute(builder: (_) => NoticeDetailScreen(notice: notice)))),
...
class NoticeDetailScreen extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final category = _label(notice['category']?.toString()), published = _formatDate(notice['publishedAt']?.toString()), expires = _formatDate(notice['expiresAt']?.toString());
    final urgent = _isUrgent(notice['category']?.toString(), notice['title']?.toString());
    return Scaffold(
      appBar: AppBar(title: const Text('Notice')),
      body: ListView(padding: const EdgeInsets.fromLTRB(20, 16, 20, 36), children: [
        Row(children: [Container(width: 48, height: 48, decoration: BoxDecoration(color: urgent ? scheme.errorContainer : scheme.primaryContainer, borderRadius: BorderRadius.circular(16)), child: Icon(urgent ? Icons.warning_amber_rounded : Icons.campaign_outlined, color: urgent ? scheme.onErrorContainer : scheme.onPrimaryContainer)), const SizedBox(width: 12), Expanded(child: Text([if (category.isNotEmpty) category, if (published != null) published].join(' • '), style: theme.textTheme.labelLarge?.copyWith(color: urgent ? scheme.error : scheme.primary, fontWeight: FontWeight.w800)))]),
        ...
        if (expires != null) ...[const SizedBox(height: 8), Text('Valid until $expires', style: theme.textTheme.bodySmall?.copyWith(color: scheme.onSurfaceVariant))],
