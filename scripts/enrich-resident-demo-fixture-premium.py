...
replace_once(
    ...
    """                      final completedJobs = offering.provider.completedJobs;\n                      final duration = offering.durationMinutes;\n                      final providerRaw = offering.raw['provider'];\n                      final providerImage = providerRaw is Map ? providerRaw['imageUrl']?.toString() : null;\n                      final membershipTier = providerRaw is Map ? providerRaw['membershipTier']?.toString() : null;\n                      final placementType = offering.raw['placementType']?.toString() ?? 'ORGANIC';\n                      final offerTitle = offering.raw['offerTitle']?.toString();\n                      final offerEndsOn = offering.raw['offerEndsOn']?.toString();\n                      final offerPricePaise = (offering.raw['discountPricePaise'] as num?)?.toInt() ?? 0;\n                      final regularPricePaise = offering.pricePaise;\n                      final hasOffer = offerPricePaise > 0 && offerPricePaise < regularPricePaise;\n                      return PremiumSurface(""",
)
...
replace_once(
    ...
    """                                SizedBox(\n                                  width: 58,\n                                  height: 58,\n                                  child: ClipRRect(\n                                    borderRadius: BorderRadius.circular(AaraagateTokens.radiusControl),\n                                    child: providerImage == null || providerImage.isEmpty\n                                        ? ColoredBox(\n                                            color: scheme.primaryContainer,\n                                            child: Center(child: Text(_offeringProviderInitial(offering), style: theme.textTheme.titleLarge?.copyWith(color: scheme.onPrimaryContainer, fontWeight: FontWeight.w800))),\n                                          )\n                                        : Image.network(\n                                            providerImage,\n                                            fit: BoxFit.cover,\n                                            errorBuilder: (_, __, ___) => ColoredBox(\n                                              color: scheme.primaryContainer,\n                                              child: Center(child: Text(_offeringProviderInitial(offering), style: theme.textTheme.titleLarge?.copyWith(color: scheme.onPrimaryContainer, fontWeight: FontWeight.w800))),\n                                            ),\n                                          ),\n                                  ),\n                                ),""",
)
