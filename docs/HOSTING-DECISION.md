...
- API readiness failure, including PostgreSQL or authentication-state/Valkey connectivity;
...
Use `/api/v1/health/live` for platform liveness so a transient database or Valkey outage does not trigger avoidable application restart loops. Use `/api/v1/health/ready` for dependency-aware routing, staging validation and external operational monitoring. The compatibility endpoint `/api/v1/health` remains available for release metadata and existing integrations.
...
Production acceptance must record:
- provider backup schedule and retention;
- encryption-at-rest status;
- point-in-time recovery window where the selected plan supports PITR;
- date/time of the most recent isolated restore drill;
- source backup/recovery point and restored target;
- verification owner and result;
- measured restore duration/RTO evidence.

The GitHub Actions backup/restore smoke proves schema-level `pg_dump`/`pg_restore` recoverability only; it is not evidence that hosted provider backups or PITR are configured.
...
4. `/api/v1/health/live` succeeds and `/api/v1/health/ready` confirms PostgreSQL and authentication-state/Valkey readiness;
5. provider backups are enabled and one isolated provider-managed restore is evidenced;
6. backup retention/encryption and PITR capability are recorded for the chosen plan;
