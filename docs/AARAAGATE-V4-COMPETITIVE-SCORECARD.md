...
Date: 2026-09-18  
Status: Repository evidence score after V4.8; V4.9 release consolidation
...
|---|---:|---:|---|
| Gate and security | >= 9.0 | **9.0** | Offline guard operations, role/tenant controls, security events, device-neutral access integration, manual fallback and regression contracts |
...
| Administration/governance | >= 8.7 | **8.8** | Privacy/audit operations, reports, migration/onboarding controls, facilities/vendors, operational dashboards and role-specific access |
...
**Overall repository evidence score: 8.84 / 10.**

The overall score is the simple arithmetic mean of the eight V4 program categories above. It exceeds the V4 release criterion of 8.6 while preserving the program's requirement that planned-only capabilities do not count.

## Important boundary

The 8.84 score reflects repository implementation and automated evidence. Production/field readiness remains exactly 8.0 because hosted staging, provider-managed backup/PITR, alert delivery, real provider credentials, representative device UAT and real access hardware certification are not proven by repository CI and are intentionally not scored as complete.
