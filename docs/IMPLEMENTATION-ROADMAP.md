...
The active competitive product-readiness program is [V4.23–V4.28](AARAAGATE-V4.23-V4.28-COMPETITIVE-READINESS.md). V4.23 UI consistency, V4.24 permission-aware AI and V4.25 payment/accounting field-readiness are repository-complete. V4.25 is implemented on `develop` through `25897c0227808c5e0cc9e457adeec640f01bc8cf`, with sandbox/live payment configuration separation, bank-statement preview safeguards, prioritized reconciliation exceptions, Resident payment-recovery visibility and optional GST/TDS operator settings. The next repository milestone is V4.26 integration ecosystem. External provider certification, hosted production evidence and field acceptance remain separate and must not be inferred from repository completion.
