...
The active competitive product-readiness program is [V4.23–V4.28](AARAAGATE-V4.23-V4.28-COMPETITIVE-READINESS.md). V4.23 UI consistency, V4.24 permission-aware AI, V4.25 payment/accounting field-readiness and V4.26 integration ecosystem are repository-complete. V4.26 is implemented on `develop` through `296f2e6aaeaf7ecdd5efe8e49584d8ae0dd83f8f`, with versioned integration contracts, capability discovery, society-scoped provider selection/audit, normalized health/retry/degradation semantics and an Admin Integration readiness workspace. The next repository milestone is V4.27 society onboarding and migration. Live provider certification, hosted production evidence, physical hardware validation and field acceptance remain separate and must not be inferred from repository completion.
