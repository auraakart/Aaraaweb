...
- OTP request remains protected by the existing per-phone Redis/Valkey request window, while the new middleware adds a client-level abuse layer. OTP verification retains its per-challenge attempt cap in addition to the client-level limiter.
