...
- **Maintenance collection percentage** — allocations recorded by the range end against the net economic value of authoritative `Receivable` records issued in the selected range. Net billed value includes append-only debit/credit/waiver adjustments recorded by the range end; collected value uses allocations net of append-only allocation reversals recorded by the range end; receivables voided after the range end remain part of that historical cohort.
- **Outstanding ageing** — authoritative receivable balance as of the selected range end (`Receivable` + append-only adjustments − net allocations after allocation reversals), grouped by current / 1–30 / 31–60 / 61–90 / 90+ day buckets.
